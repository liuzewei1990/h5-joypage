<template>
  ...
  <section class="work-view" v-bind="$attrs" :style="{ height: height }">
    ...
    <grid-layout
      ...
      :mode="mode"
      :layout.sync="workComponents"
      :row-height="rowHeight"
      :style="{ minHeight: height, '--grid': `calc(calc(100% - 0px) / 144) ${rowHeight}px` }">
      <component :is="modeCompo" v-for="(item, index) in workComponents" :key="item.i" :item="item" :index="index"></component>
    </grid-layout>
  </section>
</template>
...
<script>
  ...
  import DevWorkComponent from "../WorkComponent/index.edit.vue";
  ...
  export default {
    ...
    computed: {
      ...
      workComponents({ workModel } = state) {
