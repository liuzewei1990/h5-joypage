...
// export type LayoutItem = LayoutItemRequired & { minW?: number, minH?: number, maxW?: number, maxH?: number, moved?: boolean, static?: boolean, isDraggable?: ?boolean, isResizable?: ?boolean };
...
export function collides(l1, l2) {
  ...
  if (l1.x + l1.w <= l2.x) return false; // l1是l2的左边
  if (l1.x >= l2.x + l2.w) return false; // l1是l2的右边
  if (l1.y + l1.h <= l2.y) return false; // l1高于l2
  if (l1.y >= l2.y + l2.h) return false; // l1低于l2
  return true; // l1和l2重叠了
}

/**
 * 给定一个布局，将其压缩。这包括向下移动每个y坐标并消除项目之间的间隙。
 *
 * @param  {Array} layout Layout.
 * @param  {Boolean} verticalCompact 是否压缩布局
 *   vertically.
 * @param {Object} minPositions
 * @return {Array}       Compacted Layout.
 */
export function compact(layout, verticalCompact, minPositions) {
  // 排除掉自由布局的网格项目,只返回约束布局的网格项目
  layout = layout.filter((item) => item.isDerailed !== true);
  // 绕过静态元素
...
/**
 * 返回此布局与之冲突的第一项。
 * 我们从哪个顺序来处理这个问题似乎并不重要，尽管这可能是错误的做法。
 ...
 * @param  {Number}     [x]    X的网格单位
 * @param  {Number}     [y]    Y的网格单位
 * @param  {Boolean}    [isUserAction] 如果true 代表是用户拖动/改变大小
 */
